# --EARNED ASSIST - Preprocessing and Recipe------------------------------------------------------------------------------------------------------------------------------
library(tidymodels)
library(zoo)
library(tidyverse)
library(glmnet)
...
player_recipe <- recipe(earned_assist ~ ., data = player_df_played) %>%
  step_rm(earned_point,earned_goal,game_status) %>%
  # step_rm(earned_goal) %>%
  # step_rm(game_status) %>%
  ...
  step_smotenc(earned_assist, neighbors = 3)
# step_naomit()
...
log_facet <- cv_preds_sample %>%
  ...
  facet_wrap(~ earned_assist, ncol = 1, scales = "free") +
...
# Resample
cal_plot_breaks(cv_preds_sample, truth = earned_assist, 
                estimate = .pred_1, event_level = "first")
# Test
cal_plot_breaks(final_predictions, truth = earned_assist, 
                estimate = .pred_1, event_level = "first")


### ------Calibrate the Model on the training data---------------------
cv_cal_mod <- cal_estimate_beta(cv_preds_clean, truth = earned_assist,
                                estimate = .pred_1)
...
train_beta_cal %>% cls_met(earned_assist, .pred_1)

train_beta_cal_samp <- train_beta_cal %>% 
  slice_sample(n = min(n_rows, sample_size))
train_beta_cal_samp %>%
  cal_plot_windowed(truth = earned_assist, estimate = .pred_1, 
...
final_fit <- assessment(final_split) %>% 
  rename(earned_assist_actual = earned_assist) %>%
  bind_cols(final_cal_preds)

final_metrics_1 <- final_fit %>% cls_met(earned_assist_actual, .pred_1)
print(final_metrics_1)
# final_metrics_2 <- final_fit %>% oth_met(earned_assist, .pred_1)
...
train_model <- train_preds %>%
  ...
  bind_cols(player_df_played %>% select(earned_assist))

cv_cal_mod <- cal_estimate_beta(train_model, truth = earned_assist,
                                estimate = .pred_1)
...
unplayed_results <- unplayed_games %>%
  mutate(pred_probability = unplayed_predictions$.pred_1,
         ...
         actual_outcome = NA) %>%
  select(-c("earned_assist","earned_goal","earned_point")) %>%
...
unplayed_updated <- unplayed_results %>%
  ...
  left_join(all_boxscore_df[, c("game_id", "playerId", "teamId", 
                                "earned_assist", "earned_goal", "earned_point")],
...
# Define recipe, model, and workflow
player_recipe2 <- recipe(earned_assist ~ ., data = player_df_played) %>%
  step_rm(earned_point,earned_goal,game_status) %>%
  # step_rm(earned_goal) %>%
  # step_rm(game_status) %>%
  step_rm(all_of(c("venueUTCOffset","venueLocation","away_team_name", 
