# --EARNED POINT - Preprocessing and Recipe------------------------------------------------------------------------------------------------------------------------------
library(tidymodels)
library(zoo)
library(tidyverse)
library(glmnet)
...
# Create a new outcome variable 'earned_goal'
player_df <- all_boxscore_df

player_df <- player_df %>%
  mutate(earned_goal = factor(earned_goal, levels = c("1", "0")),
         earned_point = factor(earned_point, levels = c("1", "0")),
...
player_df <- player_df %>%
  ...
  mutate(earned_goal = as.factor(earned_goal)) %>%
  mutate(earned_goal = as.factor(earned_point)) %>%
  mutate(earned_goal = as.factor(earned_assist)) %>%
  drop_na() %>%
...
# initial_recipe <- recipe(earned_goal ~ ., data = initial_train) %>%
#   step_zv() %>%
...
player_recipe <- recipe(earned_point ~ ., data = player_df_played) %>%
  step_rm(earned_goal,earned_assist,game_status) %>%
...
# step_smotenc(earned_point, neighbors = 3)
# step_naomit()
...
saveRDS(player_recipe, file = paste0(rds_files_path, "/Data/player_recipe_point.rds"))
...
rds_files_path <- getwd()
player_recipe <- readRDS(paste0(rds_files_path, "/Data/player_recipe_point.rds"))
...
saveRDS(player_wf_log, file = paste0(rds_files_path, "/Data/point_wf_log.rds"))
saveRDS(player_fit, file = paste0(rds_files_path, "/Data/point_rocv_res_glm_fit.rds"))
...
saveRDS(final_fit, file = paste0(rds_files_path, "/Data/point_rocv_final_glm_fit.rds"))
saveRDS(final_metrics, file = paste0(rds_files_path, "/Data/point_rocv_final_glm_metrics.rds"))
saveRDS(final_predictions, file = paste0(rds_files_path, "/Data/point_rocv_final_glm_predictions.rds"))
...
rds_files_path <- getwd()
player_wf_log<- readRDS(paste0(rds_files_path,"/Data/point_wf_log.rds"))
player_fit <- readRDS(paste0(rds_files_path, "/Data/point_rocv_res_glm_fit.rds"))
final_predictions <- readRDS(paste0(rds_files_path, "/Data/point_rocv_final_glm_predictions.rds"))
gc()
...
log_facet <- cv_preds_sample %>%
  ...
  facet_wrap(~ earned_point, ncol = 1, scales = "free") +
...
# Resample
cal_plot_breaks(cv_preds_sample, truth = earned_point, 
                estimate = .pred_1, event_level = "first")
# Test
cal_plot_breaks(final_predictions, truth = earned_point, 
                estimate = .pred_1, event_level = "first")


### ------Calibrate the Model on the training data---------------------
cv_cal_mod <- cal_estimate_beta(cv_preds_clean, truth = earned_point,
                                estimate = .pred_1)
...
train_beta_cal %>% cls_met(earned_point, .pred_1)

train_beta_cal_samp <- train_beta_cal %>% 
  slice_sample(n = min(n_rows, sample_size))
train_beta_cal_samp %>%
  cal_plot_windowed(truth = earned_point, estimate = .pred_1, 
...
final_fit <- assessment(final_split) %>% 
  rename(earned_point_actual = earned_point) %>%
  bind_cols(final_cal_preds)

final_metrics_1 <- final_fit %>% cls_met(earned_point_actual, .pred_1)
print(final_metrics_1)
# final_metrics_2 <- final_fit %>% oth_met(earned_point, .pred_1)
...
# Save final metrics and predictions
saveRDS(final_fit, file = paste0(rds_files_path, "/Data/point_rocv_final_glm_fit_cal.rds"))
saveRDS(final_metrics_1, file = paste0(rds_files_path, "/Data/point_rocv_final_glm_metrics_cal.rds"))
rm(player_wf_log)
gc()
...
saveRDS(final_model, file = paste0(rds_files_path, "/Data/point_deployable_model.rds"))
rm(player_wf_log)
...
train_model <- train_preds %>%
  ...
  bind_cols(player_df_played %>% select(earned_point))

cv_cal_mod <- cal_estimate_beta(train_model, truth = earned_point,
                                estimate = .pred_1)
...
unplayed_results <- unplayed_games %>%
  mutate(pred_probability = unplayed_predictions$.pred_1,
         ...
         actual_outcome = NA) %>%
  select(-c("earned_point","earned_assist","earned_goal")) %>%
  left_join(player_names, by = "playerId")

# Save predictions
saveRDS(unplayed_results, file = paste0(rds_files_path, "/Data/unplayed_games_point_predictions.rds"))
saveRDS(unplayed_cal_preds, file = paste0(rds_files_path, "/Data/unplayed_games_point_preds_cal.rds"))
rm(unplayed_class, unplayed_cal_preds, unplayed_predictions,final_model)


#### -----Update Deployment Log------
log_file <- paste0(rds_files_path, "/Data/point_unplayed_point_predictions_glm.csv")
...
unplayed_updated <- unplayed_results %>%
  ...
  left_join(all_boxscore_df[, c("game_id", "playerId", "teamId", 
                                "earned_point", "earned_assist", "earned_goal")],
            by = c("game_id", "playerId", "teamId"))

rm(all_boxscore_df)
saveRDS(unplayed_updated, file = paste0(rds_files_path, "/Data/unplayed_games_point_actuals.rds"))
...
# Define recipe, model, and workflow
player_recipe2 <- recipe(earned_point ~ ., data = player_df_played) %>%
  step_rm(earned_goal,earned_assist,game_status) %>%
  # step_rm(earned_assist) %>%
  # step_rm(game_status) %>%
  step_rm(all_of(c("venueUTCOffset","venueLocation","away_team_name", 
...
# step_smotenc(earned_point, neighbors = 3)
# step_naomit()
saveRDS(player_recipe2, file = paste0(rds_files_path, "/Data/player_recipe_point_ml.rds"))
...
# Collect metrics from rolling CV
saveRDS(player_wf_glmnet, file = paste0(rds_files_path, "/Data/point_wf_glmnet.rds"))
saveRDS(glmnet_fit, file = paste0(rds_files_path, "/Data/point_rocv_res_glmnet_fit.rds"))
glmnet_fit <- readRDS(paste0(rds_files_path, "/Data/point_rocv_res_glmnet_fit.rds"))
...
best_glmnet <- select_best(glmnet_fit, metric = "roc_auc")
#Save Memory
rm(glmnet_fit)
gc()
...
# Save final metrics and predictions
saveRDS(final_fit, file = paste0(rds_files_path, "/Data/point_rocv_final_glmnet_fit.rds"))
saveRDS(final_metrics, file = paste0(rds_files_path, "/Data/point_rocv_final_glmnet_metrics.rds"))
saveRDS(final_predictions, file = paste0(rds_files_path, "/Data/point_rocv_final_glmnet_predictions.rds"))
...
rds_files_path <- getwd()
player_wf_pen<- readRDS(paste0(rds_files_path,"/Data/point_wf_glmnet.rds"))
player_fit <- readRDS(paste0(rds_files_path, "/Data/point_rocv_res_glmnet_fit.rds"))
final_predictions <- readRDS(paste0(rds_files_path, "/Data/point_rocv_final_glmnet_predictions.rds"))
gc()
...
# Save final metrics and predictions
saveRDS(final_fit, file = paste0(rds_files_path, "/Data/point_rocv_final_glmnet_fit_cal.rds"))
saveRDS(final_metrics_1, file = paste0(rds_files_path, "/Data/point_rocv_final_glmnet_metrics_cal.rds"))
rm(player_wf_glmnet)
gc()

### ----Fit your workflow on all training data for deployment-------------
# This creates a fully fitted model using all of player_df_played
final_model <- fit(player_wf_glmnet, data = player_df_played)
# Optionally, save this deployable model:
saveRDS(final_model, file = paste0(rds_files_path, "/Data/point_deployable_model_glment.rds"))
...
### ----Load Fitted Models and Calibration Object---------
final_model_log <- readRDS(paste0(rds_files_path, "/Data/point_deployable_model.rds"))
final_model_pen <- readRDS(paste0(rds_files_path, "/Data/point_deployable_model_glmnet.rds"))

# Assume you have a calibration model estimated on training data; if you need separate calibration for each, adjust accordingly.
# Here we use a common cv_cal_mod:
train_preds_log <- predict(final_model_log, new_data = player_df_played, type = "prob") %>% 
  rename_with(~ paste0(., "_log"))
train_class_log <- predict(final_model_log, new_data = player_df_played) %>% 
  rename_with(~ paste0(., "_log"))
train_preds_pen <- predict(final_model_pen, new_data = player_df_played, type = "prob") %>% 
  rename_with(~ paste0(., "_pen"))
train_class_pen <- predict(final_model_pen, new_data = player_df_played) %>% 
  rename_with(~ paste0(., "_pen")) 

# Bind them together along with the actual outcome
train_model_log <- bind_cols(
  train_preds_log, 
  train_class_log, 
  player_df_played %>% select(earned_point)
)

train_model_glmnet <- bind_cols(
  train_preds_pen,
  train_class_pen,
  player_df_played %>% select(earned_point))

cv_cal_mod_log <- cal_estimate_beta(train_model_log, truth = earned_point_log,
                                    estimate = .pred_1_log)
cv_cal_mod_glmnet <- cal_estimate_beta(train_model_glmnet, truth = earned_point_pen,
                                       estimate = .pred_1_pen)

### ----Define a Helper Function for Prediction--------------------
predict_model <- function(model, model_name, new_data, cal_mod) {
  # Generate raw predictions
  preds_prob <- predict(model, new_data = new_data, type = "prob")
  preds_class <- predict(model, new_data = new_data)
  
  # Apply calibration to the probabilities (if applicable)
  cal_preds <- preds_prob %>% cal_apply(cv_cal_mod)
  
  # Bind predictions and add a model name column
  preds_df <- new_data %>%
    mutate(
      pred_probability = preds_prob$.pred_1,
      pred_class = preds_class$.pred_class,
      cal_probability = cal_preds$.pred_1,
      model = model_name,
      prediction_time = Sys.time()
    )
  return(preds_df)
...
### ----Load New Data for Prediction (Unplayed Games)-----------------
# Read your unplayed games data (make sure it's in the raw format your workflow expects)
unplayed_games <- readRDS(paste0(rds_files_path, "/Data/player_df.rds")) %>% 
  filter(game_status == "unplayed")

# If your fitted workflows include the prepped recipe, they will automatically preprocess new data.
# Otherwise, you might need to bake new data with a saved prepped recipe.

### ----Generate Predictions for Each Model---------------------
preds_log <- predict_model(final_model_log, "Logistic Regression", unplayed_games)
preds_pen <- predict_model(final_model_pen, "Penalized Regression", unplayed_games)

# Combine the predictions from both models
all_predictions <- bind_rows(preds_log, preds_pen)

# add player names from your boxscore data to help interpret predictions.
player_names <- readRDS(paste0(rds_files_path, "/Data/combined_2009_2024_boxscore.rds")) %>%
  group_by(playerId) %>%
  summarise(name.default = first(name.default), .groups = "drop")
...
### ----Update Deployment Log Without Duplicates---------------------
# Define the log file path and key columns to identify a unique record.
log_file <- paste0(rds_files_path, "/Data/unplayed_predictions_models.csv")
key_cols <- c("game_id", "playerId", "teamId", "model")
pred_cols <- c("pred_probability", "pred_class", "cal_probability")

if (file.exists(log_file)) {
  old_log <- read.csv(log_file, stringsAsFactors = FALSE)
  
  # Remove any rows from old_log that match the new predictions by key
  old_log_filtered <- anti_join(old_log, all_predictions, by = key_cols)
  
  # Append new predictions
  updated_log <- bind_rows(old_log_filtered, all_predictions)
  write.csv(updated_log, log_file, row.names = FALSE)
} else {
  write.csv(all_predictions, log_file, row.names = FALSE)
}

# Optionally, save the combined predictions as an RDS file too:
saveRDS(all_predictions, file = paste0(rds_files_path, "/Data/unplayed_predictions_models.rds"))

### ----(Later) Update the Log with Actual Outcomes-----------------------
all_boxscore_df <- readRDS(paste0(rds_files_path, "/Data/combined_2009_2024_boxscore.rds"))
unplayed_updated <- all_predictions %>%
  left_join(all_boxscore_df[, c("game_id", "playerId", "teamId", "earned_goal",
                                "earned_assist", "earned_point")],
            by = c("game_id", "playerId", "teamId"))
rm(all_boxscore_df)
